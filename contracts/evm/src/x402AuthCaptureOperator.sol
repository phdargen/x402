...
/// @notice Escrow operator gated by an EIP-712 authorizer signature. `policy` must be `address(0)`.
contract x402AuthCaptureOperator is
    EIP712,
    Multicall,
    ReentrancyGuardTransient
{
    ...
    function _checkBinding(
        ...
        if (policy != address(0)) revert NonZeroPolicy();
