...
func TestToDiscoveryResource(t *testing.T) {
	t.Run("maps extracted discovery info into a catalog entry with service metadata", func(t *testing.T) {
		declared, err := bazaar.DeclareDiscoveryExtension(
			bazaar.MethodGET,
			map[string]interface{}{"city": "NYC"},
			bazaar.JSONSchema{"properties": map[string]interface{}{"city": map[string]interface{}{"type": "string"}}},
			"",
			nil,
		)
		require.NoError(t, err)
		...
		discovered, err := bazaar.ExtractDiscoveredResourceFromPaymentPayload(payloadBytes, nil, false)
		require.NoError(t, err)
		require.NotNil(t, discovered)
		...
		entry, err := bazaar.ToDiscoveryResource(*discovered, accepts, &bazaar.ToDiscoveryResourceOptions{
			LastUpdated: "2024-01-01T00:00:00Z",
		})
		require.NoError(t, err)

		assert.Equal(t, "https://api.example.com/weather", entry.Resource)
		assert.Equal(t, "Weather API", entry.ServiceName)
		assert.Equal(t, []string{"weather", "api"}, entry.Tags)
		assert.NotNil(t, entry.DiscoveryInfo)
		assert.Equal(t, "2024-01-01T00:00:00Z", entry.LastUpdated)
	})
}
