...
The server extension derives `network` from `accepts`, derives `domain`/`uri` from the configured `origin` and request path, refreshes nonce/timestamps per request, records successful payments, and validates SIWX proofs for declared HTTP routes.
...
Creates the server extension that enriches SIWX challenges, records successful payments, and verifies HTTP SIWX proofs for declared routes. Requires `origin`. Internally uses `createSIWxSettleHook` and `createSIWxRequestHook`.
...
createSIWxRequestHook({
  storage: SIWxStorage;                  // Storage for tracking paid addresses
  origin: string;                        // Public browser-visible origin (required)
