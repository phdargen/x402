...
| `s` | Client | Service code(s) — one or more client-provided attribution codes (string or array of strings) |
...
`declareBuilderCodeExtension` / `DeclareBuilderCodeExtension` validates the code format and returns the extension declaration (including the JSON Schema) for inclusion in the `PaymentRequired` response.
...
Accepts a single string or an array of strings. Attaches the client's `s` code(s) to every payment payload when registered. When the server declared `builder-code`, the core client merge also preserves the server's `a` code. Multiple codes are useful for layered clients (e.g. an MCP server acting as middleware) that need to attribute multiple participants.

<Note>Facilitators cap `s` to the first **5** entries (`MAX_SERVICE_CODES`) at settlement to bound calldata size. Extra entries are silently dropped.</Note>
