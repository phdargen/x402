...
class TestToDiscoveryResource:
    def test_maps_extracted_discovery_info_with_service_metadata(self) -> None:
        from x402.extensions.bazaar import declare_discovery_extension, to_discovery_resource
        ...
        entry = to_discovery_resource(
            discovered,
            accepts,
            last_updated="2024-01-01T00:00:00.000Z",
        )

        assert entry.resource == "https://api.example.com/weather"
        assert entry.service_name == "Weather API"
        assert entry.tags == ["weather", "api"]
        assert entry.discovery_info is not None
