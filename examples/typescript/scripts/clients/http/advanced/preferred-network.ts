...
import { requireEnv, resourceUrl } from "../../../lib/env.js";
...
const isMain = process.argv[1]?.endsWith("preferred-network.ts");
if (isMain) {
  const evmPrivateKey = requireEnv("EVM_PRIVATE_KEY") as `0x${string}`;
  const svmPrivateKey = requireEnv("SVM_PRIVATE_KEY");
  runPreferredNetworkExample(evmPrivateKey, svmPrivateKey, resourceUrl()).catch(error => {
