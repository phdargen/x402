...
import { ESCROW_ABI, ESCROW_ERRORS_ABI } from "../../../src/auth-capture/abi";
import {
  ...
  const createMockSigner = () => ({
    ...
    readContract: vi
      ...
      .mockImplementation(async (args: { functionName: string }) =>
        args?.functionName === "isValidSignature" ? ERC1271_MAGIC_VALUE : BigInt("1000000000"),
      ),
    ...
    getCode: vi
      .fn()
      .mockImplementation(async ({ address }: { address: `0x${string}` }) =>
