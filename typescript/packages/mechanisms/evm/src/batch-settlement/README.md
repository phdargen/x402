...
`createChannelManager()` throws if `voucherStore` or `authorizerSigner` is missing. Optional `{ getExtension }` attaches builder-code to scheduled transactions. The channel manager is the usual production loop: claim withdraw-pending channels, settle each `(receiver, token)` pair, and refund idle channels on the intervals you pass to `start()`. When a channel is fully claimed and idle, it is removed from the store so a later deposit can open a fresh channel.
