...
export type RedisChannelStorageOptions = {
  ...
  keyPrefix?: string;
  /** Key segment that isolates one role's records. Defaults to `"server"`. */
  scope?: string;
  lockTtlMs?: number;
...
/**
 * Redis-backed {@link ChannelStorage} with optimistic atomic updates, shared by the
 * server and facilitator voucher stores; `scope` keeps the two roles' keys apart.
 */
export class RedisChannelStorage implements ChannelStorage {
  ...
  /**
   * Creates Redis-backed channel storage.
   *
   * @param options - Redis client and optional key/scope/retry configuration.
   */
  constructor(options: RedisChannelStorageOptions) {
    this.client = options.client;
    this.keyPrefix = options.keyPrefix ?? DEFAULT_KEY_PREFIX;
    this.channelKeyPrefix = `${this.keyPrefix}:${options.scope ?? DEFAULT_SCOPE}:channel`;
