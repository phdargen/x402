...
// Lifecycle hooks clear pending reservations for normal failures. This bounded TTL
// releases channels when that cleanup cannot run or complete.
...
export interface PendingRequest {
  pendingId: string;
  signedMaxClaimable: string;
  /** `requirements.amount` seen at verify; settle may charge less but never more. */
...
/**
 * Computes the bounded expiry time of a pending reservation.
 *
 ...
 */
export function pendingExpiresAt(maxTimeoutSeconds: number | undefined, now: number): number {
 ...
 */
export function isPendingLive(pending: PendingRequest | undefined, now = Date.now()): boolean {
